...
#include <errno.h>
#include "libmeshb7.h"  // local copy
...
int64_t GmfOpenMesh(const char *FilNam, int mod, ...)
{
   ...
   if(msh->mod == GmfRead)
   {
	 ...
	 if(GetFilSiz(msh)>(1L<<31) && msh->ver < 3){
	   printf("libMeshb : binary input file size %ld is larger than version %d can handle\n",
		  GetFilSiz(msh), msh->ver);
	   longjmp(msh->err, -36);
	 }
...
static int ScaKwdTab(GmfMshSct *msh)
{
   ...
   else
   {
      ...
      do
      {
	   ...
	   printf("libMeshb : NexPos=%ld LstPos=%ld\n", NexPos, LstPos);
	   longjmp(msh->err, -30);
	 }
